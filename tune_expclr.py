"""Hyperparameter search for ExpCLR on a held-out set of subjects.

The search is staged rather than exhaustive: the target scale first, then the learning
rate around it, then the temperature. The first stage matters most because the target
scale is not free to be one: the normalized distance averages one along each row by
construction, so unless the mean of ``1 - s_ij`` also lands near one the objective keeps a
floor that no arrangement of the representation can remove.

Subjects, not windows, are split off for validation; splitting by window would put
recordings of the same child on both sides.
"""
...
DEFAULT_DESCRIPTOR = "P_madurativo"


def features_path(descriptor):
    """Returns the descriptor matrix a run of the tuner reads.

    Args:
        descriptor (str): Descriptor name.

    Returns:
        Path: Path of the matrix.
    """
...
def train_one(delta: float, lr: float, sim_max: str, epochs: int, holdout: list[str],
    ...
    if checkpoint_is_reusable(ckpt, {"delta": delta, "lr": lr, "lr_gamma": 0.99,
                                     "temperature": tau, "sim_max": sim_max,
                                     "loss_on": "projection", "dropout": 0.0,
                                     "num_epochs": epochs, "seed": seed,
                                     "descriptor": descriptor, "embedding_size": 128,
                                     "descriptor_dim": len(DESCRIPTORS[descriptor])}):
        return ckpt
