"""Evaluation of a pre-trained encoder against chronological age.

The probe is a ridge regression whose penalty is chosen by cross-validation grouped by
subject, so the choice never sees windows of the subject it is scored on. Predictions are
aggregated to recording sessions by their median before any metric is computed: windows of
one session are not independent observations, and a single session must not weigh more for
having contributed more of them.

Confidence intervals resample subjects rather than sessions or windows, because subjects
are the unit that repeats across the design.
"""
...
class RepresentationNotAvailable(TypeError):
    """Raised when a model does not expose the representation it was asked for."""


@torch.no_grad()
def extract_embeddings(model, X: np.ndarray, device, batch_size: int = 256,
                       representation: str = "projection") -> np.ndarray:
    """Reads one representation of every window from a frozen model.

    Only the contrastive encoders carry a projection head, which is why
    ``downstream.LOSS_REPRESENTATION`` names ExpCLR alone and everything else defaults to the
    embedding. Asking an autoencoder for its projection reaches its ``forward``, and that
    returns the reconstruction paired with the embedding rather than a second representation:
    the request is refused here, with the reason, instead of failing several frames later on
    whatever the tuple does not support.

    Args:
        model (torch.nn.Module): Frozen model in eval mode.
        X (np.ndarray): Windows, shape (n_windows, n_channels, n_samples).
        device (torch.device): Device to run inference on.
        batch_size (int): Windows per forward pass.
        representation (str): ``projection`` or ``embedding``.

    Returns:
        np.ndarray: One row per window.

    Raises:
        ValueError: If the representation is not one of the two known names.
        RepresentationNotAvailable: If the model does not return a tensor for it.
    """
    ...
    for start in range(0, len(X), batch_size):
        ...
        if not torch.is_tensor(z):
            raise RepresentationNotAvailable(
                f"{type(model).__name__} devuelve {type(z).__name__} en la "
                f"representación {representation!r}, no un tensor. Los modelos de "
                "reconstrucción no tienen cabeza de proyección: su forward devuelve la señal "
                "reconstruida junto al embedding, así que la representación que corresponde "
                "leer en ellos es 'embedding'."
            )
