"""Maps of the latent spaces, and the numbers that decide whether they can be read.
...
Two readings are supported on purpose. One reproduces the format of the earlier work on this
dataset, so its figures can be compared against these on equal terms: the same t-SNE, the
same clusters, the same clustering metrics. The other is what this work adds: every method
side by side in one grid, and three measures that say whether the picture can be trusted,
whether the space orders age, and how much of the arrangement is simply the identity of the
child.
...
from latent_geometry import subject_identifiability
...
# The anchor of the sweep in docs/EDA_umap_sweep_conclusiones.md.
...
def project(embeddings, technique, n_clusters=None):
    ...
    Args:
        ...
        technique (str): ``umap``, ``tsne`` or ``pca``.
    ...
    Raises:
        ProjectionError: If the technique is unknown, or if UMAP is requested and absent.
    """
    if technique == "tsne":
        ...
        try:
            ...
            raise ProjectionError(
                "UMAP was requested but umap-learn is not installed. It is declared in "
                "environment.yml; install it or choose --projection tsne."
            ) from exc
        coords = umap.UMAP(n_components=2, **UMAP_PARAMS).fit_transform(embeddings)
        settings = UMAP_PARAMS
    else:
...
def geometry_metrics(embeddings, coords, meta):
    """Measures what the figure cannot show by itself.

    Three numbers, all computed on the representation and not on the projection.
    Trustworthiness says how much the drawing distorts: the share of neighbours that appear
    together on paper and were neighbours in the original space. The silhouette over age
    says whether the space separates the four visits. Identifiability says how much of the
    arrangement is the identity of the child, holding out whole sessions so that windows of
    one recording cannot answer for each other.

    Args:
        embeddings (np.ndarray): Representation matrix.
        coords (np.ndarray): Its two-dimensional projection.
        meta (pd.DataFrame): Window metadata.

    Returns:
        dict: The three measures, plus the chance level of the last one.
    """
    sessions = meta[list(SESSION_KEYS)].astype(str).agg("_".join, axis=1).to_numpy()
    identity = subject_identifiability(embeddings, meta["subject"].to_numpy(), sessions)
    ages = meta["age"].to_numpy()
    return {
        "trustworthiness": float(trustworthiness(
            embeddings, coords, n_neighbors=TRUSTWORTHINESS_NEIGHBOURS)),
        "silhouette_age": float(silhouette_score(embeddings, ages))
        if len(np.unique(ages)) > 1 else np.nan,
        "subject_accuracy": identity["subject_accuracy"],
        "subject_chance": identity["chance"],
        "subject_times_chance": identity["times_chance"],
    }
...
def main(args):
    ...
    needs_clusters = "cluster" in args.color_by or args.metrics in ("legacy", "all")
    panels, representations, rows = {}, {}, []
    for method in args.methods:
        ...
        clusters = fit_clusters(embeddings, args.n_clusters) if needs_clusters else None
        ...
        representations[method] = (embeddings, clusters) if needs_clusters else embeddings
        if args.metrics in ("geometry", "all"):
            rows.append({"method": method, **geometry_metrics(embeddings, coords, meta)})
    ...
    if rows:
        ...
        path = os.path.join(args.save_dir, f"geometry_{args.projection}.csv")
        table.to_csv(path, index=False)
        print(table.to_string(index=False), flush=True)
        print(f"[INFO] Geometry table written to: {path}", flush=True)

    if args.metrics in ("legacy", "all"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--metrics", choices=["legacy", "geometry", "all"],
                        default="geometry",
                        help="'legacy' reproduces the clustering table of the earlier work; "
                             "'geometry' measures fidelity, age and identity.")
