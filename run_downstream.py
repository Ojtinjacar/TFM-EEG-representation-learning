...
LINEAR_PROBE_METHODS = ["PCA", "SimCLR", "AE", "MAE", "TripletLoss", "VAE", "InterFusion",
                        "ExpCLR", "ExpertFeatures"]

# The expert descriptor read straight into the task. It has no encoder to freeze or to adapt,
# so it belongs with PCA in the linear-probe list and not in the fine-tuning one: the only
# trainable part is the head, which is the same head every other method trains.
EXPERT_FEATURES_PATH = os.path.join("data", "processed", "expert_features", "P_madurativo.npy")
...
def run_pretraining(method, target, zone, frequency, test_subjects, fold_id, no_skip=False,
    ...
    if method in ["PCA", "supervised", "ExpertFeatures"]:
...
def run_downstream_experiment(method, eval_mode, target, zone, frequency, model_path, seed, test_subjects, fold_id=None, downstream_method=None, embedding_stats=None, plot_dir=None, head_dir=None):
    ...
    if method == "ExpertFeatures":
        command.extend(["--features_path", EXPERT_FEATURES_PATH])
    ...
    for target in targets:
        ...
        for eval_mode in eval_modes:
            ...
            for method in methods:
                ...
                else:  # PCA, supervised o ExpertFeatures
                    model_path = None

                # Verify that the model exists (except for the methods with no encoder)
                if method not in ["PCA", "supervised", "ExpertFeatures"] and model_path is None:
