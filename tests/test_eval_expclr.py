...
def test_a_model_without_a_projection_head_says_so():
    """An autoencoder asked for its projection must explain itself, not fail on .cpu()."""
    from eval_expclr import RepresentationNotAvailable
    from models import AttentionLSTMAutoencoder

    model = AttentionLSTMAutoencoder(input_size=64, hidden_size=8, n_channels=3, sfreq=100)
    X = np.random.default_rng(0).normal(size=(4, 3, 64)).astype(np.float32)

    with pytest.raises(RepresentationNotAvailable, match="no tienen cabeza de proyección"):
        extract_embeddings(model, X, torch.device("cpu"), representation="projection")


def test_the_same_model_reads_its_embedding_without_complaint():
    from models import AttentionLSTMAutoencoder

    model = AttentionLSTMAutoencoder(input_size=64, hidden_size=8, n_channels=3, sfreq=100)
    X = np.random.default_rng(0).normal(size=(4, 3, 64)).astype(np.float32)

    Z = extract_embeddings(model, X, torch.device("cpu"), representation="embedding")
    assert Z.shape[0] == 4
    assert Z.ndim == 2
