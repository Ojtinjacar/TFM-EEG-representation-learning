...
from checkpoint_naming import checkpoint_is_reusable
...
def test_similarity_is_bounded_with_unit_diagonal(batch):
    _, f = batch
    sim = ExpCLRLoss().expert_similarity(f)
...
def test_squared_similarity_is_the_square_of_the_linear_one(batch):
    _, f = batch
    linear = ExpCLRLoss(squared_similarity=False).expert_similarity(f)
    squared = ExpCLRLoss(squared_similarity=True).expert_similarity(f)
...
def test_train_max_distance_clamps_instead_of_flipping_sign(batch):
    _, f = batch
    tiny_max = 1e-3
    sim = ExpCLRLoss(feat_max_dist=tiny_max).expert_similarity(f)
...
def test_normalized_distance_has_unit_row_mean(batch):
    z, _ = batch
    dist = ExpCLRLoss().normalized_distance(z)
...
def test_quadratic_loss_is_non_negative(batch):
    z, f = batch
    loss = ExpCLRLoss(temperature=None)(z, f)
...
def test_identical_features_reduce_to_squared_distances():
    z = torch.randn(B, E)
    f = torch.ones(B, D)
    criterion = ExpCLRLoss(temperature=None, feat_max_dist=1.0)
...
def test_proposition_3b_large_tau_converges_to_quadratic_loss(batch):
    ...
    quadratic = ExpCLRLoss(temperature=None)(z, f).item()
    mined = ExpCLRLoss(temperature=1e6)(z, f).item()
    assert mined == pytest.approx(quadratic, rel=1e-6)


def test_proposition_3a_small_tau_converges_to_max_pair_loss(batch):
    z, f = (t.double() for t in batch)
    criterion = ExpCLRLoss(temperature=1e-5)
...
def test_mining_weights_the_worst_pairs_more_than_the_mean(batch):
    z, f = batch
    quadratic = ExpCLRLoss(temperature=None)(z, f).item()
    mined = ExpCLRLoss(temperature=1.0)(z, f).item()
...
def test_dataset_wide_max_shrinks_the_target_far_below_the_imposed_row_mean():
    ...
    def mean_target(max_dist):
        criterion = ExpCLRLoss(delta=1.0, feat_max_dist=max_dist, temperature=None)
...
def test_matching_delta_equalises_the_row_means():
    ...
    probe = ExpCLRLoss(feat_max_dist=max_dist, temperature=None)
    mean_target = float((1.0 - probe.expert_similarity(f)).mean())
    delta_star = 1.0 / mean_target

    def row_means_at(delta):
        criterion = ExpCLRLoss(delta=delta, feat_max_dist=max_dist, temperature=None)
...
def test_delta_star_approaches_one_as_distances_concentrate():
    ...
    def delta_star(matrix):
        ...
        criterion = ExpCLRLoss(feat_max_dist=max_dist, temperature=None)
...
def test_loss_is_invariant_to_rescaling_the_embedding():
    ...
    criterion = ExpCLRLoss(feat_max_dist=10.0, temperature=None)
...
def test_zero_loss_is_unreachable_at_delta_one():
    ...
    criterion = ExpCLRLoss(delta=1.0, feat_max_dist=10.0, temperature=None)
